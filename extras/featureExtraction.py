...
def getTrialFilesList(currTrialEEGSignalPath):
    trialFilesList = [f for f in os.listdir(currTrialEEGSignalPath) if f.startswith('Trial_')]
...
def concatAllTrialsByClass(marker_type, currTrialEEGSignalPath):
    ...
    file_exists = os.path.isfile(currTrialEEGSignalPath + f"All_Trials_{marker_type}.csv")
    valid_by_len = False
    if file_exists:
        outputDf_exp = pd.read_csv(currTrialEEGSignalPath + f"All_Trials_{marker_type}.csv")
        # get the length of the first row
        first_row_length = len(outputDf_exp.iloc[0]) - 1
        print("first row len: "+ str(first_row_length))
        if first_row_length == numOfSamplesToCut:
            valid_by_len = True

    print(valid_by_len)
    # check if the file exists and have the valid len - for the case of running the reparameterizeData.py
    if file_exists and valid_by_len:
